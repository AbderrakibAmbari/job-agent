...
| 022  | Dashboard V2 — inline job viewer + triage-optimised two-pane layout  | P1       | L      | MED  | 013, 009, 012 | TODO — plan drafted by Sonnet subagent 2026-07-10 against commit `59506f3`, reviewed by advisor. In-place rewrite of "Today's Matches" page: left pane = compact scrollable job list; right pane = full detail + `st.components.v1.html()` iframe of `job_url` + Applied/Not-Applying popover + auto-advance. Six phases (A–F) with per-phase commit checkpoints and a `v2_layout` sidebar toggle for rollback until operator sign-off. Public API of `nodes/tracker.py` untouched. 3 open questions BLOCK Phase C: (1) keyboard shortcuts YES/NO (would add `streamlit-shortcuts` dep), (2) iframe fallback UX when board blocks embedding, (3) fixed-height left pane (not achievable in Streamlit 1.55.0 — needs custom component). Answer before executing. |
