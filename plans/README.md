...
| 021  | Housekeeping bundle (orphan module + stale log + import + pytest pin) | P3     | S      | LOW  | 013, 019   | TODO — retire orphan `nodes/feedback_log.py`; fix `run_daily.py:65` platform label (Indeed retired, Arbeitsagentur present); lift `_default_followup_date` inline import in `dashboard.py:438` to module top via new `default_followup_date` public alias in `nodes/tracker.py`; pin `pytest==9.1.1` in `requirements.txt`. No new tests; suite baseline 236 must hold. |
